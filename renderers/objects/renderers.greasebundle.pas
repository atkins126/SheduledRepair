...
uses
  ...
  objects.greasebundle;

type
  TGreaseBundleRenderer = class(TCommonRenderer)
  public
...
{ TGreaseBundleRenderer }

procedure TGreaseBundleRenderer.CalculateColumns (AFullWidth : Cardinal);
begin
  Clear;
  AppendColumn(AFullWidth);
end;

procedure TGreaseBundleRenderer.DrawBackground(AProfile : TRendererProfile;
...
procedure TGreaseBundleRenderer.DrawText(AText : String; AProfileItem :
  TRendererProfileItem; ACanvas : TCanvas; ARect : TRect);
...
procedure TGreaseBundleRenderer.Draw(AObject : TCommonObject; AProfile : 
  TRendererProfile; ACanvas : TCanvas; ARect : TRect);
begin
  DrawBackground(AProfile, ACanvas, ARect);
  DrawText(TGreaseBundle(AObject).Grease.Supplier.Name, 
    AProfile.Items['Supplier'], ACanvas, ARect);
  DrawText(TGreaseBundle(AObject).Grease.Grade.Name, AProfile.Items['Grade'], 
    ACanvas, ARect);
  DrawText(FormatFloat('0.00', TGreaseBundle(AObject).Quantity.Count) + ' ' +
    TGreaseBundle(AObject).Quantity.Measure.Name, AProfile.Items['Quantity'],
    ACanvas, ARect);
end;
