...
type
  TNodeRenderer = class(TCommonRenderer)
  public
...
procedure TNodeRenderer.CalculateColumns (AFullWidth : Cardinal);
begin
  Clear;
  AppendColumn(AFullWidth);
end;

procedure TNodeRenderer.DrawBackground(AProfile : TRendererProfile;
...
procedure TNodeRenderer.Draw(AObject : TCommonObject; AProfile : 
  TRendererProfile; ACanvas : TCanvas; ARect : TRect);
begin
  DrawBackground(AProfile, ACanvas, ARect);
  DrawText(TNode(AObject).Name, AProfile.Items['Name'], ACanvas, ARect);
  DrawText(FormatFloat('0.00', TNode(AObject).Period.Quantity.Count) + ' ' +
    TNode(AObject).Period.Quantity.Measure.Name, AProfile.Items['Period'],
    ACanvas, ARect);
end;
